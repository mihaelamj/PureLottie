...
| --- | --- | --- |
| macOS CI | `.github/workflows/macos-ci.yml` | Full package gate: `swiftformat --lint`, `swiftlint --strict`, `swift build`, and `swift test`. |
...
The macOS full gate requires access to the private PureLayer dependency. Configure one of these repository secrets before expecting that job to pass:
...
Linux, Windows, and Wasm CI intentionally run through `scripts/ci-model-only.swift`. That script creates `.build/ci/model-only`, a temporary package containing only `Sources/LottieModel`, `Tests/LottieModelTests`, and `Tests/Fixtures`. This keeps the cross-platform model checks honest while PureLayer resolution is still a macOS full-gate concern.
