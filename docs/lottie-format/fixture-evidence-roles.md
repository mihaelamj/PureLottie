...
| `engine-divergence` | The fixture guards a place where PureLottie must match measured lottie-web behavior instead of an intuitive geometry guess. | `divergenceIDs` links the fixture to `Tools/LottieOracle/reference-divergences.json`, and committed lottie-web numeric intent records the source-frame behavior being compared. |
...
- A fixture with `engine-divergence` must declare one or more `divergenceIDs`.
  Each ID must exist in `Tools/LottieOracle/reference-divergences.json`, and the
  ledger entry must back-reference the fixture.
...
purpose, coverage, selected frame rationale, committed lottie-web trace,
reference divergence ledger links, and review evidence together. If a fixture
loses its role, purpose, or required ledger link, it is no longer evidence.
