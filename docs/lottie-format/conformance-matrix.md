...
| Layer visibility | Layer visible for `ip <= frame < op`. | 16,928 `ip`, 16,931 `op` | `inPoint`, `outPoint`. | `visibilityWindow` gates opacity in seconds. | `opacity` animation. | approx | Boundary frame tests at `ip`, `op - 1`, and `op`. |
| Hidden layers | `hd` hides normal rendering but hidden layers still matter for parents and mattes. | 204 layer `hd` keys | `isHidden`. | Binding preserves hidden parent/matte references before normal rendering skips hidden layers. | Layer omission plus binding graph. | reported | Parent and matte fixtures where source layer is hidden but semantically active. |
...
| 2D layer transform | Apply `-anchor`, scale, skew, rotation, position. Rotation is clockwise in Lottie/lottie-web matrix convention. | 16,900 anchors, 16,929 positions, 16,454 scales, 16,329 rotations | Anchor, position, scale, rotation, opacity. | Anchor point plus position, scale, rotation animations. Skew not modeled. | `Layer.anchorPoint`, `position`, `transform`, animations. | approx | Matrix comparison against lottie-web for static and animated transform cases. |
| Skew transform | `sk` and `sa` skew around skew axis between scale and rotation. | 136 layer `sk`, 137 layer `sa`, 23,555 shape `sk/sa` | Not decoded for layer or shape transforms. | Not reported by importer because model drops it. | Affine transform with skew. | gap | Model fields, ImportReport rule, and transform matrix tests. |
| 3D layer transform | `rx`, `ry`, `rz`, `or` produce 3D rotation/orientation; Lottie is 2.5D. | 103 each of `rx`, `ry`, `rz`; 101 `or` | Not decoded. | Not reported by importer because model drops it. | PureLayer 2.5D transform if available; otherwise report. | gap | Model fields and report fixtures for 3D transforms. |
| Auto orient | `ao` rotates layer to follow motion path tangent. | 14,262 `ao` keys | Not decoded. | Not reported. | Rotation animation derived from position path. | gap | Motion-path fixture with tangent orientation. |
| Blend modes | `bm` maps to CSS compositing blend modes. | 14,621 layer `bm`; many shape `bm` keys | Not decoded for layers; shape `bm` mostly not decoded. | Not reported. | PureLayer compositing/filter support or report. | gap | Blend-mode source fixtures and ImportReport assertions. |
...
| Precomp layers | `ty:0` instantiates asset layers with local timing, stretch, remap. | 2,469 | `refId`, `w`, `h`, `sr`, `st`; asset layers. | Builds asset layers; reports non-1 stretch; does not implement time remap. | Nested `Layer`, time-shifted animations. | approx | Precomp local frame tests, stretch/remap fixtures. |
| Time stretch | `sr` changes layer local time: `t' = t / sr - st`. | 14,692 `sr` keys | `stretch`. | Reports stretch when not 1, but still renders unstretched. | Animation timing scaling. | reported | Expected report plus no silent render claim. |
| Time remap | `tm` remaps layer time in seconds. | 135 layer `tm` | Not decoded. | Not reported because model drops it. | Animation local-time evaluator. | gap | Model field and report/lowering tests. |
...
| Track mattes | `tt`, `tp`, `td` connect matte layer to target layer. Hidden matte sources can still contribute. | 502 `tt`, 357 `td`, 9 `tp` | `trackMatteType`, `trackMatteParent`, `trackMatteSource`. | Binding resolves matte edges and reports unsupported matte lowering with source path/range. | Layer mask/compositing graph. | reported | Lowering tests for alpha/luma/inverted matte modes. |
| Shape scope/order | Shape arrays are programs: reverse walk, styles/modifiers apply to preceding shapes in scope. | 25,414 groups, 25,412 transforms | `LottieShape` preserves item arrays. | `DrawingProgram` walks reversed and accumulates active styles/modifiers, but remains partial. | PureDraw `Path` plus PureLayer shape/compositing layers. | approx | Golden AST/DrawingProgram tests against lottie-web shape scope examples. |
| Shape groups | Groups scope transforms, styles, modifiers; group opacity is atomic. | 25,414 | `ShapeGroup.items`. | Recurses; wraps in transparency layer when opacity static and not 1; animated group opacity reported. | Nested `Layer` for group opacity. | approx | Group opacity compositing pixel tests. |
...
| Stroke | `st` style applies color, opacity, width, caps, joins, miter, dash. | 7,456 | Color, opacity, width decoded; caps/joins/dash not modeled. | Static color/width lowered; animated color/opacity/width reported. Caps, joins, dash mostly unreported. | `ShapeLayer.strokeColor`, `lineWidth`, stroke style. | gap | Decode/report `lc`, `lj`, `ml`, `d`; cap/join/dash fixtures. |
...
| Animated scalar/vector properties | Keyframes ordered by `t`, easing via `i/o`, hold via `h`; values hold outside keyframe span. | Corpus-wide | `AnimatedDouble`, `AnimatedVector`, `LottieKeyframe`. | `ScalarTimeline` samples to PureLayer keyframes; spatial position curves linearized/reported. | `KeyframeAnimation`. | approx | Direct value evaluator tests against lottie-web for easing, hold, overshoot, spatial path. |
| Animated Bezier properties | Path keyframes interpolate vertices/tangents at frame time. | Many `sh.ks` keyframes | `AnimatedBezier` decodes keyframes. | Reports `path morph`, uses initial path. | Frame-evaluated PureDraw path animation if supported. | reported | Morph oracle tests before lowering. |
