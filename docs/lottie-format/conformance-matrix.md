...
| RenderIR frame boundary | A selected source frame must become backend-ready semantic IR before any PureLayer/PureDraw object is created. Every node retains source path, JSON path, trace identity, evaluated transform, opacity, masks/mattes, shape draws, placeholders, and diagnostics. | All composition/layer fixtures | `LottieRenderIRBuilder` emits `LottieRenderFrame` from `LottieEvaluation` without importing PureLayer. Shape geometry/style/masks/trims are evaluated at the selected frame and retain provenance. | `LottieRenderIRLowerer` is a backend pass from RenderIR to PureLayer. It does not re-run Lottie semantics; unsupported backend capabilities become `ImportReport` findings at the RenderIR node/source path. | `Layer`, `ShapeLayer`, PureDraw `Path`, and backend-gap diagnostics. | approx | Swift Testing coverage proves RenderIR emission is PureLayer-free, lowerer receives evaluated values, and backend gaps report exact RenderIR source paths. |
| lottie-web oracle | Reference frames must come from pinned lottie-web behavior, and pixel comparison is only meaningful after semantic eligibility is proven. | Selected oracle fixtures plus 857 corpus files available for expansion. | `LottieValidator` decides eligibility; `LottieRenderIRBuilder` supplies trace context per selected source frame. | `LottieFrameDump` emits PureLayer PNGs plus validation/import/RenderIR summaries; `Tools/LottieOracle` renders pinned lottie-web PNGs and writes comparison reports. | External browser harness, PNG diff, semantic trace artifacts. | modeled | Node harness tests, Swift fixture eligibility test, and reports that include frame count, selected-frame rationale, validation gate, mismatch artifacts, and RenderIR traces. |
