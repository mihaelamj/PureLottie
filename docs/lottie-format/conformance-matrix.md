...
| lottie-web oracle | Reference frames and numeric source intent must come from pinned lottie-web behavior; pixel comparison is only meaningful after semantic eligibility and numeric intent are proven. | Selected oracle fixtures plus 857 corpus files available for expansion. | `LottieValidator` decides eligibility; `LottieRenderIRBuilder` supplies trace context per selected source frame; committed `purelottie.lottie-web-intent` traces compare browser matrices, path bounds, opacity, and style facts to PureLottie source facts. | `LottieFrameDump` emits PureLayer PNGs plus validation/import/RenderIR summaries and per-frame RenderIR backend evidence; `Tools/LottieOracle` renders pinned lottie-web PNGs, extracts numeric intent JSON, and writes comparison reports. | External browser harness, lottie-web numeric trace, PNG diff, semantic trace artifacts, and backend evidence artifacts. | modeled | Node harness tests, Swift fixture eligibility test, Swift lottie-web intent comparison test, selected-frame rationale, validation gate, mismatch artifacts, RenderIR traces, and backend-evidence comparison blocking. |
