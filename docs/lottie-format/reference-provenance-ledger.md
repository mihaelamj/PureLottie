...
| Lottie format docs | `docs/lottie-format` | 14 checked-in files including this ledger | Human-readable source-intent, reversibility compiler, rendered artifact, loss taxonomy, conformance, matrix, provenance, numeric reliability, tolerance-bound, evidence role, update workflow, reference schema, and normalization confluence contracts. | Swift/Node tests pin referenced fixture counts, trace behavior, reversibility contract language, rendered artifact manifest rules, role vocabulary, provenance schema, numeric claim witness counts, tolerance derivations, and referenced workflow commands. |
