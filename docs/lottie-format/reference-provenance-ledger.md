...
| Lottie format docs | docs/lottie-format | 24 checked-in files including this ledger | Human-readable source-intent, reversibility compiler, rendered artifact, loss taxonomy, conformance, matrix, provenance, numeric reliability, tolerance-bound, evidence role, update workflow, reference schema, normalization confluence, termination cost, bounded-exhaustive round-trip, fault-injection calibration, independent conformance verifier, proof-audit-protocol, and frontend-totality-fuzzing contracts, plus the complete format reference (`lottie-format-complete.md`), bodymovin/AE source semantics (`bodymovin-source-semantics.md`), the Lottie-to-PureComposition import mapping (`lottie-import-mapping.md`), and the schema coverage verifier (`verify-coverage.sh`). | Swift/Node tests pin referenced fixture counts, trace behavior, reversibility contract language, rendered artifact manifest rules, role vocabulary, provenance schema, numeric claim witness counts, tolerance derivations, and referenced workflow commands. |
